...
def get_weather_by_coords(lat: float, lon: float, db: Session = Depends(get_db)):
    ...
    if response.status_code == 200:
        ...
        weather_data = {
            ...
            "lat": lat,
            "lon": lon,
            "date": datetime.datetime.now(pytz.timezone(data["timezone"])).strftime("%Y년 %m월 %d일 %H시 %M분 %S초"),
            ...
            "sunrise": datetime.datetime.fromtimestamp(data["sys"]["sunrise"], pytz.timezone(data["timezone"])).strftime("%H:%M:%S"),
            "sunset": datetime.datetime.fromtimestamp(data["sys"]["sunset"], pytz.timezone(data["timezone"])).strftime("%H:%M:%S")
        }
        ...
        weather_record = WeatherRecord(
            ...
            lat=lat,
            lon=lon,
            date=datetime.datetime.now(pytz.timezone(data["timezone"])),
